...
- `src/lib/gateway.ts:843-849` — `send()` on channels is a no-op for server; real-time updates to other users won't work. **Call signaling now bypasses this** via the gateway SSE bridge (`src/lib/callRealtime.ts`, Aug 14, 2026); the mock `GatewayChannel` is still used for non-call realtime.
