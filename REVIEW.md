...
|---------|------|-------|-------|
| `gateway/` | Express API gateway (Vercel) | ~5,200 | 66 |
...
- Auth routes have proper email/password validation and metadata whitelisting (`src/api/auth.ts`). Rate limits on sign-up (5/min), sign-in (20/min) and a general 1000/min default (`src/api/middleware.ts:23-27`) were historically never enforced (path-prefix mismatch + exact map lookup); **fixed Aug 23, 2026** — middleware now strips the `/api` prefix before lookup and the limiter falls back to the `*` config (`src/rate-limiting/index.ts:14`)
...
1. **Hardcoded Supabase `service_role` keys removed** — `src/infrastructure/database/infrastructureDb.ts:489-490` now retrieves keys from `process.env.INFRA_SUPABASE_URL` and `process.env.INFRA_SUPABASE_KEY`. Keys no longer embedded in source code.
2. **`.env` with `INFRA_SUPABASE_KEY` in working tree** — same service_role key accessible; `.gitignore` excludes `.env` but the file was committed before the rule was added and is **still tracked by git** (re-verified Aug 21, 2026), so the key remains exposed in repository history until rotated and untracked.
...
4. **Auth verification not domain-scoped** — **Addressed Aug 23, 2026**: `AUTH_DOMAIN` (env-overridable, default `'users'`) is now the explicit identity authority; an optional domain parameter was threaded through `getProjectCredentials`/`getSupabaseClient`/`getAnonClient`. Token *verification* intentionally remains on the authority project: the SPA uses a single global session (`tone-auth-token`) for all domains, so verifying against the path domain would reject every legitimate cross-domain request. Residual consideration: all identity lives in one project by design.
5. **`ADMIN_API_KEY` never defined in `.env`** — admin auth system is dead code (`src/auth/index.ts:215`), always returns 503
6. **`constantTimeCompare` leaks length** — **Fixed Aug 23, 2026**: both buffers are now padded to equal length before `timingSafeEqual`, so comparison time no longer reveals whether lengths matched (`src/auth/index.ts:78`).
...
- `src/notifications/index.ts:36-39` — failed notifications re-enqueue infinitely with no backoff, retry limit, or dead-letter mechanism (**fixed Aug 23, 2026**: exponential backoff, 5-attempt limit, capped dead-letter queue)
- `src/routing/router.ts:46-62` — the circuit breaker is resolved once before the retry loop; the `onRetry` project swap keeps charging failures of the swapped-to project against the original project's breaker (and `updateHealth` tracks the swapped-to ID), so failures are attributed to the wrong project and the real offender never trips (**fixed Aug 23, 2026**: breaker now resolved per attempt from the current project ID; retries skip already-failed projects)
...
- `src/api/routes.ts` — POST `/:domain` and GET `/:domain/:id` are registered twice (v1 at lines 95-201, top-level at 491-537) with minor differences; the top-level GET list additionally fans out across readable projects, which v1 lacks, and PUT/DELETE exist only under `/v1`
- `src/routing/router.ts` and `src/routing/service.ts` both implement routing logic but are disconnected
- `src/jobs/queue.ts:38-39` — single-threaded job queue silently drops concurrent `processNext()` calls (`if (this.processing) return;`); the constructor's `concurrency` option is never used

### Recent Fixes

- **Live home-feed updates without server push (Aug 23, 2026, frontend `76e48f4`)** — the home feed previously showed new posts only after a manual reload (the gateway client's `postgres_changes` listeners never fire — no server push; same root cause as the call-log caveat fixed in `e195da5`). `useHomeFeed` now self-updates three ways: a 60s poll for page 1 that runs only while the tab is visible (`document.hidden` guard), an instant re-check on `visibilitychange` when the tab comes back, and an immediate catch-up when any post-creation surface dispatches the new `tone:post-created` window event (fired after a non-scheduled post insert). Freshly-seen posts are prepended by ID — already-loaded rows and deeper pagination are left untouched, and polling failures are swallowed so they never disrupt the UI. Refactor in the same commit: post-mapping and unfollowed-group loading extracted into `mapFeedPosts()`/`loadUnfollowedGroupIds()` helpers shared by initial load and polling. tsc ✓ 0 errors in the changed file (repo has pre-existing errors elsewhere); eslint 9 pre-existing `no-explicit-any` errors remain vs 13 before.
- **Gateway security/reliability remediation + keep-alive prober (Aug 23, 2026, gateway `bfdc228`)** — five documented defects fixed in gateway source: (1) **inert rate limiter** — middleware strips the `/api` mount prefix before lookup and `check()`/`getRemaining()` fall back to the `*` default, so sign-up (5/min), sign-in (20/min) and the general 1000/min/IP limits are now actually enforced; (2) **circuit breaker mis-keying across project-swap retries** — breaker resolved per attempt from the current project ID, retries skip already-failed projects, and the shared registry object is no longer mutated; (3) **`constantTimeCompare` length leak** — buffers padded to equal length before `timingSafeEqual`; (4) **notification infinite retry** — exponential backoff, 5-attempt cap, dead-letter queue capped at 100; (5) **auth authority explicit** — `AUTH_DOMAIN` (env-overridable, default `'users'`) replaces implicit defaults, optional domain param threaded through `getSupabaseClient`/`getAnonClient`/`getProjectCredentials`; per-path-domain JWT verification deliberately NOT adopted since the SPA uses one global session (`tone-auth-token`) for all domains. Plus a new `src/keep-alive/` prober: genuine read-only queries against every active registered host (no synthetic writes) to keep Free-plan Supabase projects out of the 7-day inactivity pause — Vercel Cron fires `POST /api/keep-alive/run` at 03:00 UTC (`vercel.json`), dev runs an interval scheduler + boot round; live status at `/api/keep-alive`, history from `health_logs`. **Upgraded later the same day:** module renamed `src/project-health/`, probes staggered via a persisted per-project schedule (~10×/day per host, default; survives cold starts) driven by an every-10-min cron tick, throttled traffic-driven ticks, and dev intervals; canonical endpoint `/api/project-health` with `/api/keep-alive` kept as alias. tsc ✓ 0 errors; behavioral smoke tests ✓ (probe classification incl. paused-HTML vs DB-engaged JSON, success-only persistence split, no keys in API payloads).
- **Complete emoji catalog regenerated (Aug 22, 2026, frontend `cbfa6ea`)** — `public/emoji/emoji.json` previously covered only 665 of the 3,808 PNGs in `public/emoji/`; it is now 1:1 with the folder (entries == unique URLs == file count, zero missing). Includes all 294 regional-indicator flags under a new `Flags` category (the repo generator script excludes flags); schema unchanged (`{emoji, name, category, url}` per `EmojiData` in `src/services/emojiService.ts`); categories restricted to the picker's exact 10 tab IDs so every entry is reachable; real display names from the old entries preserved, filename codes as names elsewhere (repo convention — no offline name dataset available). Caveat: rerunning `scripts/generateEmojiJson.js` would overwrite this with a flagless 9-category file. Pushed with the provider-neutral `apply-migration.ts` rewrite (`ff82d0a`).
- **Stale `gateway/dist/` + orphan migration script scrubbed (Aug 22, 2026)** — follow-up to the Aug 21 provider-neutral pass: the gitignored `dist/` build output still held pre-scrub compiled code with baked-in provider endpoints (a hardcoded SQL-query URL template plus provider-host matching patterns); it was deleted and regenerated via `tsc` from the clean source (env-driven `INFRA_QUERY_API_TEMPLATE`, no provider URLs). The orphaned frontend `scripts/apply-migration.ts` (direct service-role client + dashboard instructions, referenced nowhere) was rewritten as an env-driven runner mirroring `apply-all-migrations.ts` (`MIGRATION_SQL_ENDPOINT` / `MIGRATION_ACCESS_TOKEN` / optional `{ref}`). Script typecheck ✓. No `.env` files modified. Both repos pushed to `ibadsixx/*`; gateway had nothing to commit (clean tree at `4966d6c`, dist is gitignored).
- **Local call preview blank after camera re-enable fixed (Aug 21, 2026, frontend `5389f01`)** — toggling video off/on restored the remote party's view (same track re-enabled via `track.enabled`, no new MediaStream) but the caller's own PiP stayed black: the `<video>` is rendered conditionally on `!isVideoOff`, so re-enabling remounted it while the attach effect only watched `[localStream]`. The effect now depends on `[localStream, isVideoOff]`, re-applies `srcObject` and calls `play()` on remount; the remote `<video>` effect gained the missing `status` dep (same conditional-render class). tsc ✓ 0 errors; eslint ✓.
- **Provider-neutral scripts + gateway endpoint scrub (Aug 21, 2026, frontend `3e8f522`, gateway `4966d6c`)** — migration helper scripts (`scripts/migrate.sh`, `scripts/apply-all-migrations.ts`) rewritten as env-driven SQL runners (`MIGRATION_SQL_ENDPOINT` + Bearer `MIGRATION_ACCESS_TOKEN`, optional `{ref}` placeholder); gateway `collectUsageMetrics()` now takes its SQL-execution endpoint from `INFRA_QUERY_API_TEMPLATE` instead of embedding it (set in Vercel env or metrics collection skips with a warning). Completes the no-provider-links/keys-in-source rule across both repos.
...
- **Realtime channel authorization (Aug 14, 2026, commit `67ce1bd`)** — `GET /api/realtime/subscribe/:channel` now returns `403` unless the channel is the caller's own `calls:<userId>` (previously any authenticated client could subscribe to `calls:<victim>` and read/spoof call signaling); both subscribe and publish validate the `calls:<id>` channel format (`400`). Production caveat updated Aug 17–21, 2026: the hub is no longer strictly process-local — publishes are relayed across gateway instances via a shared Supabase Realtime broadcast bus (`983bf60`, deploy fixes `1ca14a3`/`2a26b40`), so cross-instance signaling works; remaining limits are the 300s `maxDuration` SSE kill (reconnect gap on calls >~5 min) with no replay of in-flight signals, and `delivered` counting only local subscribers (so `delivered=0` can false-positive across instances).
- **Realtime SSE bridge + ICE endpoint (Aug 14, 2026)** — `ibadsixx/gateway` commit `29f9559`: new `src/realtime/channelHub.ts` (in-memory SSE fan-out hub) and `src/api/realtime.ts` mounted at `/api/realtime` — `GET /api/realtime/subscribe/:channel` (SSE, auth, `init` connId, 25s heartbeat, cleanup on close), `POST /api/realtime/publish` (`{channel, event, payload, excludeConnId}` → `{ok, delivered}`), `GET /api/realtime/ice-servers` (Google STUN + TURN from `TURN_URL`/`TURN_USERNAME`/`TURN_CREDENTIAL`, OpenRelay fallback). Powers the frontend's call signaling. Pre-existing `busboy` type errors (`TS2307`/`TS7006` in `src/api/routes.ts`) are unchanged from the base commit. (The "process-local hub" limitation noted at the time was superseded Aug 17, 2026 by the cross-instance bus relay — see previous bullet.)
- **Removed hardcoded Supabase keys** — `src/infrastructure/database/infrastructureDb.ts:489-490` now retrieves `INFRA_SUPABASE_URL` and `INFRA_SUPABASE_KEY` from `process.env` instead of embedding `service_role` keys in fallback data. Committed keys eliminated from source code.
- **`/api/system/databases` removed** — the endpoint that exposed all service keys has been deleted from the gateway source (`src/api/routes.ts:273`); the remaining `/api/system/*` endpoints except `/health` are gated behind `authenticateAdmin` (still 503 while `ADMIN_API_KEY` is undefined).
...
- **Feature flags re-enabled on registry refresh** — gateway `e3ee66d` (`src/dev.ts`): the 60s `refreshRegistry()` now calls `featureFlags.enable(domain)` after each reload, so domains registered in the live infra DB activate on the next refresh cycle in long-lived processes. Scope correction (Aug 23, 2026): this refresh loop exists only in the local dev server — the Vercel handler (`api/[...slug].ts`) enables flags once per cold start, so production still picks up new domains on instance recycling, not via a live refresh.
...
- **Aliased FK joins fixed — message avatars showed "U" instead of photos (Aug 21, 2026, frontend `7540a9a`)** — `parseJoinSpec` in `src/lib/gateway.ts` kept the whole `alias:table` header for `alias:table!fk(columns)` selects, so the join resolver fetched `/api/sender_profile:profiles` (gateway → 400) and every message row silently lacked `sender_profile`; per-message avatars fell back to the `'U'` letter (most visible on mobile where the open chat fills the screen). The parser now strips the alias prefix and uses it as the result key, repairing all aliased FK joins app-wide.
...
- **Legacy rows backfilled (data-only)** — post-deploy test calls still wrote generic envelopes because the browser ran the pre-`442631e` bundle (rows carried no `callerId`, which new code always includes); all 6 existing call-log rows in the live messages project were PATCHed via its service key to add caller/receiver ids+names, so history renders personalized labels immediately. New calls store names automatically once clients run `442631e+`.
...
- Impressive PostgREST-compatible gateway client (`src/lib/gateway.ts:296-673`) with `.eq()`, `.neq()`, `.like()`, `.or()`, `.order()`, `.range()`, `.single()`, `.maybeSingle()`
...
2. **Auth tokens + private keys both in `localStorage`** — `src/lib/gateway.ts:858` stores `access_token` and `refresh_token` alongside encryption keys
...
4. **Regex injection** — `src/lib/gateway.ts:132-139` interpolates user input into `new RegExp()` without escaping special characters, potential ReDoS

### High-Severity Issues

- `src/lib/gateway.ts:478-482` — client-side filtering fetches entire tables into the browser when gateway ignores query params. Catastrophic for performance at scale.
...
- `src/lib/gateway.ts:843-849` — `send()` on channels is a no-op for server; real-time updates to other users won't work. **Call signaling now bypasses this** via the gateway SSE bridge (`src/lib/callRealtime.ts`, Aug 14, 2026); the mock `GatewayChannel` is still used for non-call realtime. Client-side polling mitigations for table changes: call-log messages bridge inserts via a `tone:call-log` window event (`e195da5`), and the home feed polls page 1 every 60s + catches up on tab focus and a `tone:post-created` event (`76e48f4`, Aug 23, 2026).
...
- `src/App.tsx:48` — `QueryClient` created at module scope, persists between test runs
...
**React Query nearly unused despite being a dependency** — `@tanstack/react-query` is installed (`package.json:44`) and `QueryClientProvider` wraps the app (`App.tsx:50`), but only 1 of the 89 hooks (`src/hooks/useMusicLibrary.ts`) uses `useQuery`/`useMutation`; the rest use manual `useState`/`useEffect`/`fetch` — no caching, no dedup, no stale-while-revalidate, no background refetching.

**TypeScript strictness disabled** — `tsconfig.app.json:25` has `"strict": false`; `tsconfig.json` sets `"noImplicitAny": false` (line 4) and `"strictNullChecks": false` (line 13):
```
"noImplicitAny": false,
...
**500+ `console.log` statements** in production code (538 across `src/`) — `useFileUpload.ts` (5), `useEditorProject.ts` (14), `src/lib/storage.ts` (8), among others.
...
| System endpoints admin-gated but dead | `ADMIN_API_KEY` unset → 503; `/api/system/databases` removed (Aug 2026) | `src/api/routes.ts:266` | — | **Medium** |
...
| Auth credentials not domain-scoped | Cross-domain auth bypass | `src/auth/index.ts:51` | — | **Addressed Aug 23, 2026** — authority made explicit (`AUTH_DOMAIN`); per-path-domain verification rejected as it would break the SPA's single global session |
| `/api/rpc` auth-gated & users-only | RPC-backed features fall back to client-side emulation (full-table fetches); proxy exists (`routes.ts:344`, mounted at `routes.ts:421`) but needs a token + domain routing | gateway | `usePeopleYouMayKnow` | **Medium** |
| Backend host paused/unreachable | Gateway healthy but a domain's host is down — partial outage invisible to `/api/system/health`; Free-plan Supabase hosts auto-pause after 7 days of low DB activity (paid hosts are immune) | All data routes (per-project) | Generic failures; retry UX treats it like a bug | **Mitigated Aug 23, 2026** — gateway keep-alive prober (`/api/project-health`, alias `/api/keep-alive`) sends genuine read-only queries to every registered host (~10 staggered probes/host/day via the persisted scheduler) and exposes per-host ACTIVE/UNREACHABLE status; classify plan + pause reason for full automation (`tone-api-gateway.md` → Host Project Pausing Risk) |
...
| No server push for table changes (`postgres_changes` never fire) | New rows invisible until reload; open chats and feed went stale | gateway client lib registers listeners nothing delivers | Call-log `tone:call-log` event bridge (`e195da5`); home-feed 60s poll + tab-focus + `tone:post-created` catch-up (`76e48f4`) | **Mitigated Aug 23, 2026** — per-feature client workarounds; a general change-notification channel (SSE or broadcast relay) remains open |
...
| Local video preview blank after camera re-enable | Caller's own PiP stays black; remote party sees video fine (same track reused) | — | PiP `<video>` rendered conditionally on `!isVideoOff` + attach effect missing that dep | **Fixed Aug 21, 2026 (`5389f01`)** — effect deps `[localStream, isVideoOff]` re-attach `srcObject` + `play()` on remount; no new MediaStream; remote video effect gains `status` dep |
...
5. **~~Fix the auth credential scoping~~** — **Addressed Aug 23, 2026**: `AUTH_DOMAIN` authority made explicit, domain param threaded through client methods; per-path-domain JWT verification intentionally not adopted (would break the SPA's single global session)
...
14. **Fix gateway bugs** — monitoring random values (remaining); notification infinite retry, circuit breaker mis-keying, and inert rate limiting **fixed Aug 23, 2026** in the gateway
